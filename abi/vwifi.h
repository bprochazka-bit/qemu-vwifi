...
/* This header is the wire protocol and nothing else: no ioctls, no
 * Linux-only includes.  QEMU device models include it and QEMU builds on
 * Windows and macOS hosts, where <sys/ioctl.h> does not exist.  The
 * host module's /dev/vwifi-ctl ioctl ABI lives in vwifi_host_ioctl.h. */
 ...
 * With dynamic radios this is only the default name of the optional
 * boot-time radio (module param default_radio=1); each dynamically
 * created radio gets its own /dev/<name> node (see below). */
#define VWIFI_CHRDEV_NAME        "vwifi"

/* Hello/registration message, sent by a peer right after it connects:
 *
 *   [uint32 length (network byte order)]   -- bytes following
 *   [uint32 VWIFI_HELLO_MAGIC]             -- HOST byte order
 *   [node_id\0]
 *   [uint8 flags]                          -- optional
 *
 * Note the asymmetry: only the outer length prefix is in network byte
 * order.  The magic, like every field of vwifi_frame_hdr, goes out in
 * host byte order -- the hub compares the raw uint32 without ntohl().
 * Sending htonl(VWIFI_HELLO_MAGIC) on a little-endian host produces a
 * hello the hub silently ignores.
 *
 * Magic spells "VWIR" on the wire (vwifi registration).
 *
...
#define VWIFI_HELLO_MAGIC          0x52495756  /* "VWIR" */
...
/* One length-prefixed message on the wire: 4 (length prefix) + header +
 * max frame.  A reader that only needs to hold a single message in
 * flight can size its buffer to exactly this. */
#define VWIFI_MSG_SLOT_SIZE      \
    (4 + VWIFI_HDR_SIZE + VWIFI_MAX_FRAME_SIZE)

/* Default reassembly buffer size: one message.
 *
 * Peers that de-aggregate A-MPDU bursts want more than one slot -- the
 * medium delivers a run of subframes back-to-back, and a one-message
 * buffer forces one read() per subframe.  Those peers define their own
 * multiple of VWIFI_MSG_SLOT_SIZE rather than changing this default;
 * see VWIFI_ATH9K_RXBUF_SIZE in devices/ath9k. */
#define VWIFI_RXBUF_SIZE         VWIFI_MSG_SLOT_SIZE
